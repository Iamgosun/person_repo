...
def resolve_prediction_path(run_dir: Path | None, pred_path: str | None) -> Path:
    if pred_path is not None:
        path = Path(pred_path).expanduser().resolve()
        if not path.exists():
            raise FileNotFoundError(f"找不到 --pred 指定的文件: {path}")
        return path

    if run_dir is None:
        raise ValueError("必须提供 --run-dir 或 --pred 其中之一。")

    path = (run_dir / "eval" / "id" / "test" / "predictions.pt").resolve()
    if not path.exists():
        raise FileNotFoundError(f"找不到默认 test predictions: {path}")
    return path


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--run-dir", default=None, help="Path to one run_dir")
    parser.add_argument("--pred", default=None, help="Optional explicit path to predictions.pt")
    parser.add_argument("--mc-samples", type=int, default=1000)
    parser.add_argument("--bins", type=int, default=15)
    parser.add_argument("--seed", type=int, default=0)
    args = parser.parse_args()

    run_dir = Path(args.run_dir).expanduser().resolve() if args.run_dir is not None else None
    pred_path = resolve_prediction_path(run_dir=run_dir, pred_path=args.pred)

    obj = torch.load(pred_path, map_location="cpu")
    required = ["probs", "labels", "preds", "logits_mean", "logits_var"]
    missing = [k for k in required if k not in obj]
    if missing:
        raise KeyError(f"Missing keys in {pred_path}: {missing}. Available: {list(obj.keys())}")
    ...
    print("\n[说明]")
    print("- 当前脚本默认读取 run_dir/eval/id/test/predictions.pt；")
