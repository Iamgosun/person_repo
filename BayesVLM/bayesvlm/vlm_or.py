...
# 定义了一个数据容器 EncoderResult，用于存储编码器的三种输出
    # embeds：最终的嵌入向量。
    # activations：投影层之前的激活值（中间特征）。
    # residuals：残差连接的值。??
@dataclass
class EncoderResult:
    embeds: torch.Tensor
...
# 数据容器  概率化的 logits
    # softmax(...): 实现了概率 Softmax。它有两种模式：
        # 采样模式：从分布中采样多次，计算平均 Softmax 概率。
        # 近似模式：使用多类 Probit 近似（Multiclass Probit approximation）直接计算，避免昂贵的采样。
    # sample_probas(...): 专门用于从分布中采样概率。
    # expected_aleatoric_entropy(...): 计算预期偶然熵（Expected Aleatoric Entropy）。这是衡量模型“数据不确定性”的指标，熵越高，模型越不确定。
    # cross_entropy(...): 计算带不确定性的交叉熵损失。
...
# CLIPTextEncoder
# 继承自 torch.nn.Module，封装了 HuggingFace 的 CLIPTextModelWithProjection。
# Forward 逻辑：接收文本 Batch -> Tokenize -> 通过 Transformer -> 取池化输出（Pooled Output） -> 投影层 -> 得到 Embedding。
# 如果 return_activations=True，则返回 EncoderResult 对象，包含 Embedding 和中间的激活值（用于后续计算方差）。
...
#定义了 CLIP 和 SIGLIP 类，实现了概率化相似度计算。
#定义了模型参数 logit_scale（温度系数）和 logit_bias（偏置）。两个类变量用于标记投影层是否有偏置项（SigLIP 通常有，CLIP 通常无）。
#source/target_covariance: 协方差矩阵对象（用于描述权重的不确定性），类型为 KroneckerFactorizedCovariance（外部定义的类）。

#  确定性计算 (标准 CLIP) _compute_logits
#  概率化计算 (Smith et al. 方法) _compute_probabilistic_logits_smith
        # 数学原理：
        # 它利用 Kronecker Factorized Covariance（克罗内克分解协方差）来近似权重的不确定性。
        # 它计算了两个随机变量（Source 和 Target Embeddings）点积的期望（Expected Value）和方差（Variance）。
        # 公式逻辑：
        # 期望相似度：基于归一化后的均值向量计算。
        # 方差相似度：结合了均值的平方和协方差项（source_diag_cov, target_diag_cov），推导出了相似度分数的不确定性。
        # 返回：ProbabilisticLogits 对象，包含预测的均值和方差。
...
class CLIP(torch.nn.Module):
    ...
    # 把“确定的 embedding 向量”升级成“带高斯不确定性的 embedding 随机变量”，
    # 然后近似计算两个 embedding 之间 cosine similarity 的均值和方差。

    # activations 用来算 embedding 的协方差   Σ 由 activations + posterior covariance 推出来
    # embeds 用来算 embedding 的均值   𝜇  由 embeds 给出
    # 𝑊 不是固定参数，而是有后验分布(投影层) 
    # embedding 𝑧 也就有分布， 用了 Kronecker factorization 近似协方差 
    # 所以均值和方差都要跟着变换。
