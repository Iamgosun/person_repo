...
def _get_projection_dim(model_name_or_path: str, local_files_only: bool = False) -> int:
    """
    支持两种情况：
    1) 传入 Hugging Face repo id，例如 laion/CLIP-ViT-B-32-laion2B-s34B-b79K
    2) 传入本地目录，例如 ./models/clip-vit-b32
    """
...
class CLIPTextEncoder(torch.nn.Module):
    ...
    def from_huggingface(
        ...
        if projection_dim is None:
            projection_dim = _get_projection_dim(
                model_name,
                local_files_only=local_files_only,
            )
...
class SiglipImageEncoder(torch.nn.Module):
    ...
    def enable_gradients(
        ...
        # 修正：原版这里访问 self.vision_encoder.encoder.layers 会报错，
        # 实际 encoder 在 self._raw_vision_model.vision_model.encoder.layers
...
class CLIP(torch.nn.Module):
    ...
    def _compute_probabilistic_logits_smith(
        ...
        source_diag_cov = (
            torch.einsum("ij,jk,ik->i", source_activations, source_covariance.A_inv, source_activations)[:, None]
            * source_B_factor
        )
        target_diag_cov = (
            torch.einsum("ij,jk,ik->i", target_activations, target_covariance.A_inv, target_activations)[:, None]
            * target_B_factor
        )
