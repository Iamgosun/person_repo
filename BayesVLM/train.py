...
# 1. 读参数：数据集、模型名、Hessian目录
# 2. 根据模型类型创建 transform 和 datamodule
# 3. 加载 image encoder / text encoder / VLM wrapper
# 4. 从磁盘读入 A_img, B_img, A_txt, B_txt
# 5. 优化 lambda_img / lambda_txt（先验精度）
# 6. 用 A/B 和 lambda 计算图像、文本投影层的后验协方差
# 7. 把协方差塞回 VLM
# 8. 预计算测试图像特征、类别文本特征
# 9. 得到 probabilistic logits（均值 + 方差）
# 10. 用 probit 近似把 logits 分布变成分类概率
# 11. 计算 ACC / NLPD / ECE
...
def print_class_counts(ds, split_name="train"):
    ...
    print(f"===== {split_name} =====")
    print(f"num_classes: {len(class_names) if class_names is not None else len(counter)}")

    for class_id in sorted(counter.keys()):
        name = class_names[class_id] if class_names is not None else str(class_id)
        print(f"{class_id:3d} | {name:25s} | {counter[class_id]}")
...
def main(
    ...
    DATA_BASE_DIR="/root/autodl-tmp/BayesVLM/datasets"
    f = DataModuleFactory(
    ...
    class Config:
        # 1. 定义类属性
        model_name_or_path = "./models/clip-vit-b32"
        adapter_name = "LP"
    ...
    # 实例化配置对象
    cfg = Config()
    class_names, counter=print_class_counts(dm.test_ds, "test")

    vlmadapter=VLMAdapter(cfg,class_names,text_covariance= cov_txt)
