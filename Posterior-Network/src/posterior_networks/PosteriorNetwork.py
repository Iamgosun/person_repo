...
# 作用：在贝叶斯推断中，  N 通常作为先验强度或“预算”。
# 'id': 直接使用样本数。
# log(N+1) ，防止某些类别样本过多导致主导。
# 'one': 忽略样本数量差异，视为平衡数据集。
...
#自定义的架构（如 VGG, ResNet, Linear）和密度估计网络（如 NormalizingFlowDensity）
...
class PosteriorNetwork(nn.Module):
    ...
    def forward(self, input, soft_output, return_output='hard', compute_loss=True):
        ...
        # 无密度估计 (no_density=True) 通过 Softmax 得到概率分布。
        if self.no_density:  # Ablated model without density estimation
            ...
            zk = self.batch_norm(zk)  # 特征提取与归一化
            ...
            #利用 density_estimation 模块计算潜在特征 的对数概率密度 
